    ...
    try:
        ...
        for entity in entities:
            ...
            if isinstance(entity, parser.Junk):
                continue
            storage[unicode(entity)] = entity.val
...
def main():
    ...
    # Allow to pipe the output in Bash
    sys.stdout = codecs.getwriter(locale_module.getpreferredencoding())(sys.stdout)
    ...
    moz_languages = {}
    parse_file(
        os.path.join(en_path, 'toolkit', 'chrome', 'global', 'languageNames.properties'),
        moz_languages
    )
    # Read languages from CLDR
    cldr_localenames_path=os.path.join(script_folder, 'node_modules', 'cldr-localenames-full', 'main')
    ...
    for locale, language in moz_languages.iteritems():
        if locale not in cldr_languages:
            missing_languages.append(u'{}: {}'.format(locale, language))
        elif language != cldr_languages[locale]:
            different_values_lang.append(u'{}\n  CLDR: {}\n  Mozilla: {}'.format(locale, cldr_languages[locale], language))
    ...
    moz_regions = {}
    parse_file(
        os.path.join(en_path, 'toolkit', 'chrome', 'global', 'regionNames.properties'),
        moz_regions
    )
    # Read regions from CLDR
    with open(os.path.join(cldr_localenames_path, 'en', 'territories.json')) as data_file:
        json_data = json.load(data_file)
    ...
    for region_code, region_name in moz_regions.iteritems():
        ...
        if code not in cldr_regions:
            missing_regions.append(u'{}: {}'.format(code, region_name))
        elif region_name != cldr_regions[code]:
            different_values_reg.append(u'{}\n  CLDR: {}\n  Mozilla: {}'.format(code, cldr_regions[code], region_name))
    ...
    row = [
        ...
        round(len(different_values_lang) / float(len(moz_languages)) * 100, 2) if len(moz_languages) > 0 else 0,
        len(moz_regions),
        len(different_values_reg),
        round(len(different_values_reg) / float(len(moz_regions)) * 100, 2) if len(moz_regions) > 0 else 0
    ]
    csv_rows.append(','.join(map(str, row)))

    # Check if there are Mozilla locales missing from CLDR
    cldr_supported_locales = []
    for locale_path in glob('{}/*/'.format(cldr_localenames_path)):
    ...
    for locale in moz_locales:
        ...
        if locale_mapping.get(locale, locale) not in cldr_supported_locales:
            if locale in seed_locales:
                missing_locales.append('{} (available in seed)'.format(locale))
            elif locale_code in seed_locales:
                missing_locales.append('{} (available in seed as {})'.format(locale, locale_code))
            else:
    ...
    for locale in moz_locales:
        ...
        moz_languages = {}
        different_values_lang = []
        l10n_file = os.path.join(l10n_path.format(locale), 'toolkit', 'chrome', 'global', 'languageNames.properties')
        if os.path.isfile(l10n_file):
            parse_file(l10n_file, moz_languages)

            # Read languages from CLDR
            cldr_localenames_path=os.path.join(script_folder, 'node_modules', 'cldr-localenames-full', 'main')
            with open(os.path.join(cldr_localenames_path, locale_mapping.get(locale, locale), 'languages.json')) as data_file:
                json_data = json.load(data_file)
            cldr_languages = json_data['main'][locale_mapping.get(locale, locale)]['localeDisplayNames']['languages']
            ...
            for locale_code, language in moz_languages.iteritems():
                ...
                cldr_language = cldr_languages[locale_code].lower() if locale_code in cldr_languages else False;
                if cldr_language and language != cldr_language:
                    different_values_lang.append(u'{}\n  CLDR: {}\n  Mozilla: {}'.format(locale_code, cldr_language, language))
            different_values_lang.sort()
            if different_values_lang:
                print('\nDifferent values:')
                print('\n'.join(different_values_lang))
        ...
        moz_regions = {}
        different_values_reg = []
        l10n_file = os.path.join(l10n_path.format(locale), 'toolkit', 'chrome', 'global', 'regionNames.properties')
        if os.path.isfile(l10n_file):
            parse_file(l10n_file, moz_regions)

            # Read regions from CLDR
            with open(os.path.join(cldr_localenames_path, locale_mapping.get(locale, locale), 'territories.json')) as data_file:
                json_data = json.load(data_file)
            cldr_regions = json_data['main'][locale_mapping.get(locale, locale)]['localeDisplayNames']['territories']
        ...
        # Append data
        row = [
            '{} ({})'.format(locale, locale_mapping.get(locale, locale)),
            len(moz_languages),
            len(different_values_lang),
            round(len(different_values_lang) / float(len(moz_languages)) * 100, 2) if len(moz_languages) > 0 else 0,
            len(moz_regions),
